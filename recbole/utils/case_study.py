...
def full_sort_scores(uid_series, model, test_data, device=None):
    ...
    if not test_data.is_sequential:
        ...
        for i, hist_iid in enumerate(history_item):
            # print(i)
            # print("i:", i, "\tShape of hist_iid:", np.shape(hist_iid))
            try:
        ...
        history_item = [element for element in history_item if element is not None]

        # print("Len of history_item", len(list(history_item)))

        history_row = torch.cat(full_like_list)
        """
